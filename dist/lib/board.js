...
exports.isPieceHanging =
    exports.getDefenders =
    exports.getAttackers =
    exports.pieceValues =
    exports.promotions =
        void 0;
...
function isPieceHanging(lastFen, fen, square) {
    ...
    // Validate that the pieces exist before accessing their properties
    if (!lastPiece || !piece) {
        console.error(
            `Invalid piece detected. lastPiece: ${lastPiece}, piece: ${piece}, square: ${square}`
        );
        return false; // If either piece is undefined, return false (not hanging)
    }
    ...
    if (
        exports.pieceValues[lastPiece.type] >=
            exports.pieceValues[piece.type] &&
    ...
    if (
        attackers.some(
            (atk) =>
                exports.pieceValues[atk.type] < exports.pieceValues[piece.type]
        )
    ) {
        return true;
    }
    ...
    if (attackers.length > defenders.length) {
        ...
        for (let attacker of attackers) {
            minAttackerValue = Math.min(
                exports.pieceValues[attacker.type],
                minAttackerValue
            );
        }
        if (
            exports.pieceValues[piece.type] < minAttackerValue &&
            defenders.some(
                (dfn) => exports.pieceValues[dfn.type] < minAttackerValue
            )
        ) {
            return false;
        }
